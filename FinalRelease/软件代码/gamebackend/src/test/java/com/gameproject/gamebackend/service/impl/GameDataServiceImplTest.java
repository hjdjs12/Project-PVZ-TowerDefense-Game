...
//////////////////////////////////// 项目内用到的函数
    ...
    void register() {
        ...
        assertEquals("用户名已存在",gameDataService.register(gameData));
        // 错误的手机格式
        gameData.setUsername(username3);
        gameData.setPhone(w_phone1);
        assertEquals("手机号码格式错误",gameDataService.register(gameData));
        gameData.setPhone(w_phone2);
        assertEquals("手机号码格式错误",gameDataService.register(gameData));
        // 错误的邮箱格式
        gameData.setPhone(phone);
        gameData.setEmail(w_email1);
        assertEquals("邮箱格式错误",gameDataService.register(gameData));
        gameData.setEmail(w_email2);
        assertEquals("邮箱格式错误",gameDataService.register(gameData));
        // 成功注册
        gameData.setEmail(email);
        assertEquals("注册成功",gameDataService.register(gameData));
    }
}