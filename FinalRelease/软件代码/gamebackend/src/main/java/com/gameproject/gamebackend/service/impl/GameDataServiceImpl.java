...
public class GameDataServiceImpl implements GameDataService {
    ...
    public String register(GameData gameData) {
        // 用户名已经存在
        if (gameDataMapper.isExisted(gameData.getUsername()) == 1) return "用户名已存在";
        // 手机号码格式错误
        if (gameData.getPhone().length() != 11) return "手机号码格式错误";
        // 邮箱格式错误
        if (gameData.getEmail().isEmpty()) return "邮箱格式错误";
        if (!gameData.getEmail().contains("@qq.com")) return "邮箱格式错误";
        if (gameData.getEmail().indexOf('@') != gameData.getEmail().lastIndexOf('@')) return "邮箱格式错误";
